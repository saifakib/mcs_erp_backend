...
module.exports.auditReports = async (req, res, next) => {
  ...
  try {
    ...
    if (hrid == '' && deptid == '' && tdate == '' && fdate == '' && month == '') {
      console.log(rows);
      ...
      if (fdate || tdate) {
        ...
        const response = rows.filter((log) =>
          log.EMPLOYE_ID == hrid && (fdate && (log.ENTRY_DAY != null && (new Date(`${log.ENTRY_DAY}`) >= new Date(`${fdate}`))) || tdate && (log.ENTRY_DAY != null && (new Date(`${log.ENTRY_DAY}`) <= new Date(`${fdate}`))))
      ...
      if (fdate || tdate) {
        const response = rows.filter((log) =>
          log.DEPARTEMENT_ID == Number(deptid) && (fdate && (log.ENTRY_DAY != null && (new Date(`${log.ENTRY_DAY}`) >= new Date(`${fdate}`))) || tdate && (log.ENTRY_DAY != null && (new Date(`${log.ENTRY_DAY}`) <= new Date(`${fdate}`))))
        )
        res.json(createResponse({ logs: addMinutesDiff(response) }));
      }
      else if (month) {
      ...
      const response = rows.filter((log) =>
       fdate && (log.ENTRY_DAY != null && (new Date(`${log.ENTRY_DAY}`) >= new Date(`${fdate}`))) || tdate && (log.ENTRY_DAY != null && (new Date(`${log.ENTRY_DAY}`) <= new Date(`${fdate}`)))
      );
      res.json(createResponse({ logs: addMinutesDiff(response)}));
    }
  } catch (err) {
