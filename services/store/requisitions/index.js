...
module.exports.singleStoreRoles = (id) =>
  Execute(
    `SELECT R.ROLE_ID, R.ROLE_NAME, E.NAME_ENGLISH, D.DEPARTEMENT, DG.DESIGNATION from STR_ROLE R left outer join HRM.EMPLOYEE E ON E.EMPLOYE_ID = R.EMP_ID LEFT OUTER JOIN HRM.DEPARTMENT_LIST D ON D.DEPARTEMENT_ID = E.DEPARTEMENT_ID LEFT OUTER JOIN HRM.DESIGNATION DG ON  DG.DESIGNATION_ID = E.DESIGNATION_ID WHERE ROLE_ID = ${Number(
      id
    )}`
  );
