...
const stockStatus = () =>
  Execute(
    `SELECT SP.PROID, SP.PRONAME, SP.PRONAMETWO, SP.PROQTY, C.CATEGORYEN FROM STR_STOREPRODUCTS SP LEFT OUTER JOIN STR_CATEGORIES C ON SP.PROCATE = C.CAT_ID ORDER BY PROID ASC`
  );

const getProductSummariesByProductid = (PROID) =>
  Execute(
    `SELECT PS.SUMMDATE, PS.PRODUCTNAME, PS.INTIALQTY, PS.NEWADDQTY, PS.TOTALBALANCE, PS.TOTALOUT, PS.PRESENTBALANCE, P.NAME, P.DESIGNATION, P.DEPARTMENT, C.CATEGORYBN, C.CATEGORYEN FROM STR_PRODUCTSUMMARIES PS LEFT OUTER JOIN STR_PROFILES P ON PS.REQUISITIONFOR = P.PROFID LEFT OUTER JOIN STR_CATEGORIES C ON PS.PROCAT = C.CAT_ID  WHERE PS.PRODUCTID = ${PROID} ORDER BY PROSUMID ASC`
  );
...
const getProductSummariesBySummMonthAndCatId = (SUMMMONTH, CATID) =>
  Execute(
    `SELECT PS.SUMMDATE, PS.PRODUCTNAME, PS.INTIALQTY, PS.NEWADDQTY, PS.TOTALBALANCE, PS.TOTALOUT, PS.PRESENTBALANCE, P.NAME, P.DESIGNATION, P.DEPARTMENT, C.CATEGORYBN, C.CATEGORYEN FROM STR_PRODUCTSUMMARIES PS LEFT OUTER JOIN STR_PROFILES P ON PS.REQUISITIONFOR = P.PROFID LEFT OUTER JOIN STR_CATEGORIES C ON PS.PROCAT = C.CAT_ID  WHERE PS.SUMMMONTH = ${SUMMMONTH} AND PS.PROCAT = ${Number(
      CATID
    )} ORDER BY PROSUMID ASC`
  );

const getProductSummariesByCatId = (CATID) =>
  Execute(
    `SELECT PS.SUMMDATE, PS.PRODUCTNAME, PS.INTIALQTY, PS.NEWADDQTY, PS.TOTALBALANCE, PS.TOTALOUT, PS.PRESENTBALANCE, P.NAME, P.DESIGNATION, P.DEPARTMENT, C.CATEGORYBN, C.CATEGORYEN FROM STR_PRODUCTSUMMARIES PS LEFT OUTER JOIN STR_PROFILES P ON PS.REQUISITIONFOR = P.PROFID LEFT OUTER JOIN STR_CATEGORIES C ON PS.PROCAT = C.CAT_ID  WHERE PS.PROCAT = ${Number(
      CATID
    )} ORDER BY PROSUMID ASC`
  );
/*-------------------------------END SELECT --------------------------------*/
