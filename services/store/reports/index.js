...
const getRequisitionsByProduct = (proid) => Execute(`SELECT PR.PRODATE, SP.PRONAME, SP.PRONAMETWO, PR.PROREQUQTY, PR.APROQTY, 
PR.PREMARKS, E.NAME_BANGLA, E.NAME_ENGLISH, DL.DEPARTEMENT, DG.DESIGNATION FROM STR_PROREQUISITIONS PR
LEFT OUTER JOIN HRM.EMPLOYEE E 
ON PR.HRIDNO = E.EMPLOYE_ID 
LEFT OUTER JOIN STR_STOREPRODUCTS SP 
...
LEFT OUTER JOIN HRM.DESIGNATION DG 
ON E.DESIGNATION_ID = DG.DESIGNATION_ID 
WHERE PR.PROID = ${Number(proid)} AND PR.APROQTY != 0`);


const getRequisitionsByProductAndDate = (proid, fdate, tdate) => Execute(`SELECT PR.PRODATE, SP.PRONAME, SP.PRONAMETWO, PR.PROREQUQTY, PR.APROQTY, 
PR.PREMARKS, E.NAME_BANGLA, E.NAME_ENGLISH, DL.DEPARTEMENT, DG.DESIGNATION FROM STR_PROREQUISITIONS PR
...
WHERE PR.PROID = ${Number(proid)} AND PR.PRODATE >= '${fdate}' AND PR.PRODATE <= '${tdate}' AND PR.APROQTY != 0`);


const getRequisitionsByPerson = (hrid) => Execute(`SELECT PR.PRODATE, SP.PRONAME, SP.PRONAMETWO, PR.PROREQUQTY, PR.APROQTY, 
PR.PREMARKS, E.NAME_BANGLA, E.NAME_ENGLISH, DL.DEPARTEMENT, DG.DESIGNATION FROM STR_PROREQUISITIONS PR
LEFT OUTER JOIN HRM.EMPLOYEE E 
ON PR.HRIDNO = E.EMPLOYE_ID 
LEFT OUTER JOIN STR_STOREPRODUCTS SP 
...
const getRequisitionsByPersonAndDate = (hrid, fdate, tdate) => Execute(`SELECT PR.PRODATE, SP.PRONAME, SP.PRONAMETWO, PR.PROREQUQTY, PR.APROQTY, 
PR.PREMARKS, E.NAME_BANGLA, E.NAME_ENGLISH, DL.DEPARTEMENT, DG.DESIGNATION FROM STR_PROREQUISITIONS PR
LEFT OUTER JOIN HRM.EMPLOYEE E 
ON PR.HRIDNO = E.EMPLOYE_ID 
LEFT OUTER JOIN STR_STOREPRODUCTS SP 
...
LEFT OUTER JOIN HRM.DESIGNATION DG 
ON E.DESIGNATION_ID = DG.DESIGNATION_ID 
WHERE PR.HRIDNO = ${Number(hrid)} AND PR.PRODATE >= '${fdate}' AND PR.PRODATE <= '${tdate}' AND PR.APROQTY != 0`);


// User Requisition Report Start
//-------------------------------//
const getUserReqReportByDate = (hrid, fdate, tdate) => Execute(`SELECT distinct(RE.REQID), RE.REQUITIME, RE.REQUIDATE,
...
LEFT OUTER JOIN HRM.DESIGNATION DG
ON E.DESIGNATION_ID = DG.DESIGNATION_ID
WHERE RE.PROFILEHRID = ${Number(hrid)} AND RE.REQUIDATE >= '${fdate}' AND RE.REQUIDATE <= '${tdate}'`);


const getUserReqReportByMonth = (hrid, month) => Execute(`SELECT distinct(RE.REQID), RE.REQUITIME, RE.REQUIDATE,
SUM(PR.PROREQUQTY) OVER(PARTITION BY RE.REQID) AS TOTAL_PROREQUQTY,
SUM(PR.APROQTY) OVER(PARTITION BY RE.REQID) AS TOTAl_APROQTY,
E.NAME_BANGLA, E.NAME_ENGLISH, DL.DEPARTEMENT, DG.DESIGNATION FROM STR_REQUISITIONS RE
LEFT OUTER JOIN STR_PROREQUISITIONS PR
...
LEFT OUTER JOIN HRM.DESIGNATION DG
ON E.DESIGNATION_ID = DG.DESIGNATION_ID
WHERE RE.PROFILEHRID = ${Number(hrid)} AND REQUIMONTH = '${month}'`);


const getUserReqReportByYear = (hrid, year) => Execute(`SELECT distinct(RE.REQID), RE.REQUITIME, RE.REQUIDATE,
SUM(PR.PROREQUQTY) OVER(PARTITION BY RE.REQID) AS TOTAL_PROREQUQTY,
SUM(PR.APROQTY) OVER(PARTITION BY RE.REQID) AS TOTAl_APROQTY,
E.NAME_BANGLA, E.NAME_ENGLISH, DL.DEPARTEMENT, DG.DESIGNATION FROM STR_REQUISITIONS RE
LEFT OUTER JOIN STR_PROREQUISITIONS PR
...
//view
const getUserSingleReqView = (hrid, requestid) => Execute(`SELECT RE.REQUISITIONNO, PR.REQUIID, SP.PRONAME, SP.PRONAMETWO, RE.REQUITIME, RE.REQUIDATE, PR.PROREQUQTY, PR.APROQTY, PR.PREMARKS,
