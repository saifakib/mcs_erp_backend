...
const getProductEntriListMrr = (SUP_ID, MRRNO) =>
  Execute(
    `SELECT * FROM STR_PRODUCTENTRILISTS LT LEFT OUTER JOIN STR_STOREPRODUCTS S ON LT.PRODUCTIDNO = S.PROID LEFT OUTER JOIN STR_UNITS U ON S.PRODUNIT = U.UNIT_ID WHERE PRODUCTFROM = ${SUP_ID} AND MRRNUMBER=${MRRNO}`
  );
