...
const getRecentMonthSupply = (
  ...
  return Execute(
    `SELECT PE.MRRNNO, PE.PROINID, S.SUP_ID, S.SUPPLIER, PE.ENTRIDATE FROM STR_PRODUCTENTRIES PE LEFT OUTER JOIN STR_SUPPLIERS S ON PE.SUPPLIER = S.SUP_ID WHERE ENTRIMONTH = '${month}' AND (PE.MRRNNO || LOWER(S.SUPPLIER)) LIKE '${searchr}' ORDER BY PE.MRRNNO DESC OFFSET ${offsetr} ROWS FETCH NEXT ${limitr} ROWS ONLY`
  );
};
...
const getProductEntiresFirstMrr = (SUP_ID, MRRNO) =>
  Execute(
    `SELECT * FROM STR_PRODUCTENTRIES WHERE SUPPLIER = ${SUP_ID} AND MRRNNO= ${MRRNO} FETCH NEXT 1 ROWS ONLY`
  );

const getProductEntriListss = (SUP_ID, MRRNO, date) =>
  Execute(
    `SELECT * FROM STR_PRODUCTENTRILISTS LT LEFT OUTER JOIN STR_STOREPRODUCTS S ON LT.PRODUCTIDNO = S.PROID LEFT OUTER JOIN STR_UNITS U ON S.PRODUNIT = U.UNIT_ID WHERE PRODUCTFROM = ${SUP_ID} AND MRRNUMBER=${MRRNO} AND PRODATE = '${date}'`
  );

const getProductEntriListMrr = (SUP_ID, MRRNO) =>
  Execute(
    `SELECT * FROM STR_PRODUCTENTRILISTS LT LEFT OUTER JOIN STR_STOREPRODUCTS S ON LT.PRODUCTIDNO = S.PROID LEFT OUTER JOIN STR_UNITS U ON S.PRODUNIT = U.UNIT_ID LEFT OUTER JOIN STR_CATEGORIES C ON C.CAT_ID = S.PROCATE WHERE PRODUCTFROM = ${SUP_ID} AND MRRNUMBER=${MRRNO}`
  );

//LEFT OUTER JOIN STR_UNITS U ON S.PRODUNIT = U.UNIT_ID
