...
WHERE R.REQ_STATUS = ${Number(status)}`)
  ...
  const selectRequisitionById = (req_id) => ExecuteIT(`SELECT VE.NAME_ENGLISH, VE.MOBILE_PHONE, VE.DEPARTEMENT, VE.DESIGNATION, R.REQ_ID, TO_CHAR(R.REQ_DATE,'DD-MM-YYYY') AS REQ_DATE, TO_CHAR(R.REQ_DATE,'HH12:MI AM') AS REQ_TIME, R.USER_REMARKS, PR.PRO_REQ_ID, PR.QUNTITY, PR.APR_QTY, PL.PRODUCT_ID, PL.PRODUCT_NAME FROM REQUISITION R
  LEFT OUTER JOIN PRO_REQUISITION PR ON R.REQ_ID = PR.REQ_ID
