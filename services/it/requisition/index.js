...
const selectIndProductList = (str_pro_id, status) =>
  ExecuteIT(
    `SELECT * FROM IND_PRODUCT WHERE STR_PRO_ID = ${Number(
      str_pro_id
    )} AND STATUS = ${Number(status)}`
  );

// isPending
const selectUserReqIsPending = (user_id) =>
  ExecuteIT(`SELECT REQ_ID, TO_CHAR(REQ_DATE, 'DD-MM-YYYY') AS LAST_REQ_DATE,
(CASE 
...
FROM REQUISITION 
WHERE REQ_ID=(SELECT max(REQ_ID) FROM REQUISITION WHERE HR_ID = ${Number(
    user_id
  )})`);
...
const selectUserRequisitions = (user_id) =>
  ...
  LEFT OUTER JOIN hrm.DESIGNATION DE ON DE.DESIGNATION_ID = E.DESIGNATION_ID WHERE R.HR_ID=${Number(
    user_id
  )}`);
...
const selectUserAcceptActiveRequisitions = (user_id) =>
  ...
  LEFT OUTER JOIN MODELS M ON M.MODEL_ID = SP.MODEL_ID
  LEFT OUTER JOIN CATEGORIES C ON C.CATEGORY_ID = PL.CATEGORY_ID
  WHERE R.REQ_STATUS = 2 AND R.HR_ID =  ${Number(
    user_id
  )} AND I.STATUS = ${Number(0)}`);
...
const selectStatusRequisitions = (status, ts, given) => {
  if (ts) {
    ...
    WHERE R.REQ_STATUS = ${Number(status)} AND R.GIVEN = ${Number(
      given
...
const selectRequisitionById = (req_id, status) =>
  ExecuteIT(`SELECT VE.NAME_ENGLISH, VE.MOBILE_PHONE, VE.DEPARTEMENT, VE.DESIGNATION, R.REQ_ID, 
...
const selectAllDetailsRequisitionById = (req_id) =>
  ExecuteIT(`SELECT VE.NAME_ENGLISH, VE.MOBILE_PHONE, VE.DEPARTEMENT, VE.DESIGNATION, R.REQ_ID, 
  TO_CHAR(R.REQ_DATE,'DD-MM-YYYY') AS REQ_DATE, TO_CHAR(R.REQ_DATE,'HH12:MI AM') AS REQ_TIME, IP.UNIQUE_V, B.BRAND_NAME, U.UNIT_NAME,
  R.USER_REMARKS, PR.PRO_REQ_ID, PR.QUNTITY, PR.APR_QTY, PL.PRODUCT_ID, PL.PRODUCT_NAME,
  sum(PR.QUNTITY) over(partition by (PR.REQ_ID)) as T_REQQUNTITY, sum(PR.APR_QTY) over(partition by (PR.REQ_ID)) as T_APR_QTY FROM REQUISITION R
...
const selectIndRequisitionById = (ind_pro_req_id) =>
  ExecuteIT(
    `SELECT IND_PRODUCT_ID FROM IND_PRO_REQUISITION WHERE IND_PRO_ID = ${Number(
      ind_pro_req_id
    )}`
  );
...
const insertRequisitionInfo = ({ hrid, status, userRemarks }) =>
  ExecuteIT(
    `INSERT INTO REQUISITION (HR_ID, REQ_STATUS, USER_REMARKS) VALUES (${Number(
      hrid
    )}, ${Number(status)}, '${userRemarks}') RETURN REQ_ID INTO :id`,
...
const insertSummaries = (data) =>
  ExecuteIT(
    `INSERT INTO PRODUCT_SUMMARIES (PRO_ID, STR_PRO_ID, TOTAL_OUT, SUM_TYPE) VALUES (${Number(
      data.PRO_ID
    )}, ${Number(data.STR_PRO_ID)}, ${Number(data.TOTAL_OUT)}, '${
      data.SUM_TYPE
    }') RETURN SUMMARY_ID INTO :id`,
    { id: { type: oracledb.NUMBER, dir: oracledb.BIND_OUT } }
  );
...
const updateRequisition = (data, given, OTP) => {
  if (data.REQ_STATUS == 2) {
    if (given) {
      return ExecuteIT(
        `UPDATE REQUISITION SET REQ_STATUS = ${Number(
          data.REQ_STATUS
        )}, GIVEN = ${Number(1)} WHERE REQ_ID = ${Number(data.REQ_ID)}`
      );
    } else {
      return ExecuteIT(
        `UPDATE REQUISITION SET REQ_STATUS = ${Number(
          data.REQ_STATUS
        )} WHERE REQ_ID = ${Number(data.REQ_ID)}`
      );
    }
  } else {
    if (given) {
      return ExecuteIT(
        `UPDATE REQUISITION SET REQ_STATUS = ${Number(data.REQ_STATUS)},
          STR_REMARKS = '${data.STR_REMARKS}', GIVEN = ${Number(
          1
        )}, OTP = ${Number(OTP)} WHERE REQ_ID = ${Number(data.REQ_ID)}`
      );
    } else {
      return ExecuteIT(
        `UPDATE REQUISITION SET REQ_STATUS = ${Number(data.REQ_STATUS)},
          STR_REMARKS = '${data.STR_REMARKS}', OTP = ${Number(
          OTP
        )} WHERE REQ_ID = ${Number(data.REQ_ID)}`
      );
    }
...
const updateReqGivenByIT = (id) =>
  ExecuteIT(`UPDATE REQUISITION SET GIVEN = ${Number(1)} WHERE REQ_ID = ${id}`);
...
const updateIndProReq = (ind_pro_req_id, status, current_date) => {
  if (current_date) {
    return ExecuteIT(
      `UPDATE IND_PRO_REQUISITION SET STATUS = ${Number(
        status
      )}, RETURN_DATE = TO_DATE('${current_date}', 'YYYY-MM-DD') WHERE IND_PRO_ID = ${Number(
        ind_pro_req_id
      )}`
    );
  } else {
    return ExecuteIT(
      `UPDATE IND_PRO_REQUISITION SET STATUS = ${Number(
        status
      )} WHERE IND_PRO_ID = ${Number(ind_pro_req_id)}`
    );
  }
};
